...
class PoissonTrainer(FlexibleNeuralProbeTrainer):
    ...
    def train_loss(self, minibatch: torch.Tensor, minibatch_assignment: torch.Tensor,
        ...
        for _ in range(self.mc_samples):
            ...
            # Compute negative log-likelihood
            loss_rf += loss_fn(class_scores, minibatch_assignment).detach() * self.variational_family.logprob(mask)  # noqa
        ...
        rf_scale = 1
        loss = loss_mc + entropy_scale * loss_entropy + rf_scale * loss_rf + weights_regularization
